...
bool LoopDevice::SetName(const std::string& name) {
  ...
  strncpy(reinterpret_cast<char*>(info.lo_file_name),
          name.c_str(), LO_NAME_SIZE);
  return SetStatus(info);
}

bool LoopDevice::Detach() {
  if (loop_ioctl_.Run(GetDevicePath(), LOOP_CLR_FD,
                      0, kLoopDeviceIoctlFlags) != 0) {
...
std::unique_ptr<LoopDevice>
LoopDeviceManager::GetAttachedDeviceByNumber(int device_number) {
...
LoopDeviceManager::SearchLoopDevicePaths(int device_number) {
  ...
  if (device_number != -1) {
    auto loopdev_path = rootdir.Append(base::StringPrintf("loop%d",
                                                          device_number));
    if (base::PathExists(loopdev_path))
      devices.push_back(CreateLoopDevice(device_number,
                                         GetBackingFile(loopdev_path)));
  } else {
    // Read /sys/block to discover all loop devices.
    base::FileEnumerator loopdev_enum(rootdir,
                                      false /*recursive*/,
    ...
    for (auto loopdev = loopdev_enum.Next(); !loopdev.empty();
      ...
      if (dev_number != -1)
        devices.push_back(CreateLoopDevice(dev_number,
                                           GetBackingFile(loopdev)));
    }
  }
  return devices;
}
