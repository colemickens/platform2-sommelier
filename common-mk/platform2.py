...
class Platform2(object):
  ...
  def configure_gn(self):
    ...
    uses = {}
    for flag in self.iuse_flags:
      uses[flag.lstrip('-+')] = flag[0] == '+'
    for flag in self.use_flags:
      if flag not in uses:
        logging.notice("use flag %s not in iuse", flag)
      uses[flag] = True
    use_args = [
        '%s=%s' % (x.replace('-', '_'), str(uses[x]).lower()) for x in uses]
...
def GetParser():
  ...
  parser.add_argument('--iuse_flags', action='split_extend',
                      help=('list of all use flags used within the ebuild '
                            'except for arch USE flags such as arm.'))
