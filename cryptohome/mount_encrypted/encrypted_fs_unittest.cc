...
class EncryptedFsTest : public ::testing::Test {
 public:
  EncryptedFsTest() :
    ...
    CHECK(
        base::CreateDirectory(tmpdir_.GetPath().AppendASCII("home/chronos")));

    // Setup EncryptedFs with temp directory.
    encrypted_fs_ = std::make_unique<EncryptedFs>(
        tmpdir_.GetPath(), &platform_,
        &loopdev_manager_, &device_mapper_);

    // Setup paths to check.
    brillo::SecureBlob digest =
      cryptohome::CryptoLib::Sha256(brillo::SecureBlob(tmpdir_.GetPath()
                                                              .value()));
    std::string hex = cryptohome::CryptoLib::BlobToHex(digest);
    dmcrypt_name_ = "encstateful_" + hex.substr(0, 16);
    dmcrypt_device_ = base::FilePath(base::StringPrintf("/dev/mapper/%s",
                                                       dmcrypt_name_.c_str()));
...
TEST_F(EncryptedFsTest, RebuildStateful) {
  ...
  EXPECT_CALL(platform_, GetBlkSize(_, _)).WillOnce(
      DoAll(SetArgPointee<1>(40920000), Return(true)));
...
TEST_F(EncryptedFsTest, OldStateful) {
  ...
  EXPECT_CALL(platform_, Mount(dmcrypt_device_, mount_point_, _, _, _))
            .WillOnce(Return(true));
  EXPECT_CALL(platform_, Bind(_, _)).Times(2).WillRepeatedly(Return(true));
  EXPECT_CALL(platform_, Unmount(_, _, _)).Times(3)
...
TEST_F(EncryptedFsTest, DevmapperTeardown) {
  ...
  EXPECT_EQ(device_mapper_.GetTable(dmcrypt_name_).ToBlob(),
            empty_table.ToBlob());
}
