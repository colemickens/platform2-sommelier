...
TEST(PasswordAuthDelegateTest, NullInitialization) {
  PasswordAuthDelegate delegate("");
...
TEST(PasswordAuthDelegateTest, SerializationTest) {
  std::string expected_auth("\x40\x00\x00\x09"  // session_handle = TPM_RS_PW
                            ...
                            "secret",           // password
                            15);
  PasswordAuthDelegate delegate("secret");
...
TEST(PasswordAuthDelegateTest, ParseGoodParams) {
...
TEST(PasswordAuthDelegateTest, ParseBadParams) {
  std::string auth_response("\x00\x00"  // nonceTpm = zero length buffer
                            "\x01"      // session_attributes = continueSession
                            "\x00\x06"  // password length
                            "secret",   // password
...
TEST(PasswordAuthDelegateTest, EncryptDecrypt) {
  PasswordAuthDelegate delegate("secret");
