...
bool ExternalDisplay::SendMessage(const std::vector<uint8>& body) {
  ...
  return delegate_->PerformI2COperation(&ioctl_data);
}

bool ExternalDisplay::ReceiveMessage(std::vector<uint8>* body) {
  DCHECK(body);
