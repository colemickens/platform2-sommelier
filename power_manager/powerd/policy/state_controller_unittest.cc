...
// Test for correct functioning of bright display wake locks, which
// should not let the screen dim or turn off, and should not let the
// system suspend.
TEST_F(StateControllerTest, BrightDisplayWakeLocks) {
  Init();

  // A bright wake lock should prevent the screen from dimming, turning
  // off, or performing the standard idle action.
  ASSERT_TRUE(controller_.AcquireDisplayWakeLock(WAKE_LOCK_BRIGHT));
  // If we step time here, the dim/off timeouts should not trigger.
  ASSERT_FALSE(StepTimeAndTriggerTimeout(default_ac_screen_dim_delay_));
  ASSERT_FALSE(StepTimeAndTriggerTimeout(default_ac_screen_off_delay_));
  // This should be true because the idle action should be triggered,
  ...
  // Releasing the bright lock after the dim delay passes should
  // result in a screen dim, but only after release.
  ASSERT_FALSE(StepTimeAndTriggerTimeout(default_ac_screen_dim_delay_));
  EXPECT_EQ(kNoActions, delegate_.GetActions());
  ASSERT_TRUE(controller_.ReleaseDisplayWakeLock(WAKE_LOCK_BRIGHT));
  EXPECT_EQ(kScreenDim, delegate_.GetActions());
  ResetLastStepDelay();
  controller_.HandleUserActivity();
  // Clear actions.
  ...
  // Same for the off delay and suspend delay.
  ASSERT_TRUE(controller_.AcquireDisplayWakeLock(WAKE_LOCK_BRIGHT));
  ASSERT_FALSE(StepTimeAndTriggerTimeout(default_ac_screen_off_delay_));
  EXPECT_EQ(kNoActions, delegate_.GetActions());
  ASSERT_TRUE(controller_.ReleaseDisplayWakeLock(WAKE_LOCK_BRIGHT));
  ...
  ASSERT_TRUE(controller_.AcquireDisplayWakeLock(WAKE_LOCK_BRIGHT));
  // Idle action should have fired here.
  ASSERT_TRUE(StepTimeAndTriggerTimeout(default_ac_suspend_delay_));
  EXPECT_EQ(kNoActions, delegate_.GetActions());
  ASSERT_TRUE(controller_.ReleaseDisplayWakeLock(WAKE_LOCK_BRIGHT));
  EXPECT_EQ(JoinActions(kScreenDim, kScreenOff, kSuspend, NULL),
...
// Test that dim wake locks let the screen dim, but not let it turn off or let
// the system suspend.
TEST_F(StateControllerTest, DimDisplayWakeLocks) {
  Init();

  // A dim wake lock should prevent the screen from turning
  // off or performing the standard idle action, but the screen
  // can dim.
  ASSERT_TRUE(controller_.AcquireDisplayWakeLock(WAKE_LOCK_DIM));
  ASSERT_TRUE(StepTimeAndTriggerTimeout(default_ac_screen_dim_delay_));
  EXPECT_EQ(kScreenDim, delegate_.GetActions());
  // The off timeout should not trigger.
  ASSERT_FALSE(StepTimeAndTriggerTimeout(default_ac_screen_off_delay_));
  // This should be true because the idle action should be triggered,
  // but it should be a no-op.
  ASSERT_TRUE(StepTimeAndTriggerTimeout(default_ac_suspend_delay_));
  ...
  // Releasing the dim lock after the off delay passes should
  // result in the screen turning off, but only after release.
  ASSERT_TRUE(StepTimeAndTriggerTimeout(default_ac_screen_dim_delay_));
  ASSERT_FALSE(StepTimeAndTriggerTimeout(default_ac_screen_off_delay_));
  EXPECT_EQ(kScreenDim, delegate_.GetActions());
  ASSERT_TRUE(controller_.ReleaseDisplayWakeLock(WAKE_LOCK_DIM));
  EXPECT_EQ(kScreenOff, delegate_.GetActions());
  ResetLastStepDelay();
  controller_.HandleUserActivity();
  // Clear actions.
  ...
  EXPECT_EQ(kScreenDim, delegate_.GetActions());
  ASSERT_TRUE(controller_.ReleaseDisplayWakeLock(WAKE_LOCK_DIM));
  EXPECT_EQ(JoinActions(kScreenOff, kSuspend, NULL), delegate_.GetActions());
  ResetLastStepDelay();
  controller_.HandleUserActivity();
  // Clear actions.
  EXPECT_EQ(JoinActions(kScreenUndim, kScreenOn, NULL), delegate_.GetActions());
...
// Tests for the interaction of bright and dim display wake locks.
TEST_F(StateControllerTest, BrightAndDimDisplayWakeLocks) {
  Init();
  ...
  // If we let the screen-off timeout pass, we should still have a
  // bright screen, and no timeouts should trigger.
  ASSERT_FALSE(AdvanceTimeAndTriggerTimeout(default_ac_screen_off_delay_));
  EXPECT_EQ(kNoActions, delegate_.GetActions());

  // Releasing the bright lock should let the screen dim.
  ASSERT_TRUE(controller_.ReleaseDisplayWakeLock(WAKE_LOCK_BRIGHT));
  EXPECT_EQ(kScreenDim, delegate_.GetActions());

  // Releasing the dim lock should let the screen go off.
  ASSERT_TRUE(controller_.ReleaseDisplayWakeLock(WAKE_LOCK_DIM));
  EXPECT_EQ(kScreenOff, delegate_.GetActions());
}

// Test that we error out properly on invalid wake locks.
TEST_F(StateControllerTest, InvalidDisplayWakeLocks) {
  Init();

  // Wake locks that are out of range can't be acquired or released.
  ASSERT_FALSE(controller_.AcquireDisplayWakeLock(
      static_cast<DisplayWakeLockType>(-1)));
  ASSERT_FALSE(controller_.ReleaseDisplayWakeLock(
      static_cast<DisplayWakeLockType>(-1)));
  ASSERT_FALSE(controller_.AcquireDisplayWakeLock(WAKE_LOCK_TYPES));
  ASSERT_FALSE(controller_.ReleaseDisplayWakeLock(WAKE_LOCK_TYPES));

  // You can't release wake locks of valid types if you don't have them.
