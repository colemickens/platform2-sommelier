...
class CameraDeviceAdapter : public camera3_callback_ops_t {
 public:
  ...
  bool Start(ReprocessEffectCallback reprocess_effect_callback);
 ...
 private:
  ...
  // A mapping from the reprocessing buffer. The values of the map hold the
  // handle for stream buffers to point to, and the handle ID of
  // original input buffer. Note that the key cannot be used to pass the handle
  // because it is const'ed while the buffer pointer of camera3_stream_buffer_t
  // is not.
  std::unordered_map<buffer_handle_t, std::pair<buffer_handle_t, uint64_t>>
      reprocess_handles_;
