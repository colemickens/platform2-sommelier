...
ExecuteTaskBase::ExecuteTaskBase(const char *name, int priority):
    TaskThreadBase(name),
...
ExecuteTaskBase::~ExecuteTaskBase()
{
    HAL_TRACE_CALL(CAMERA_DEBUG_LOG_LEVEL1);
    deInit();
}

status_t ExecuteTaskBase::init()
{
    HAL_TRACE_CALL(CAMERA_DEBUG_LOG_LEVEL1);
    return initMessageThread();
}

status_t ExecuteTaskBase::executeTask(ProcTaskMsg &/*msg*/)
{
    // no-op for base class
    return NO_ERROR;
}

status_t ExecuteTaskBase::handleMessageIterationDone(Message &msg)
{
    // no-op for base class
    UNUSED(msg);
    return NO_ERROR;
...
void
ExecuteTaskBase::messageThreadLoop()
{
    mThreadRunning = true;
...
status_t
ExecuteTaskBase::requestExitAndWait()
{
    HAL_TRACE_CALL(CAMERA_DEBUG_LOG_LEVEL1);
    Message msg;
 ...
 */
uint8_t ExecuteTaskBase::analyzeIntent(ProcTaskMsg &pMsg)
{
    ...
    case ANDROID_CONTROL_CAPTURE_INTENT_MANUAL:
        LOG2("%s: Request intent: ANDROID_CONTROL_CAPTURE_INTENT_MANUAL", __FUNCTION__);
        break;
    default:
...
status_t ExecuteTaskBase::allocateInterBuffer(bool isFallback,
 ...
 */
status_t ExecuteTaskBase::setIntermediateBuffer(
        bool isFallback,
 ...
 */
bool ExecuteTaskBase::isVideoStream(CameraStream *stream)
{
    bool display = false;
    bool videoEnc = false;
