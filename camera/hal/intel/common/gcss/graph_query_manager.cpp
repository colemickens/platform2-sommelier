                                        ...
                                        bool strict)
{
    ...
    if (ret != css_err_none) {
        LOGV("Invalid XML. Settings tag has no children.");
        return ret;
    }
        ...
        GraphConfigNode *ret_node)
{
    ...
    if (ret == css_err_none && !is_src_port_enabled) {
        ...
        LOGV("Src port %s disabled, skip dst and peer processing",
                      src_node_name.c_str());
        return css_err_none;
    }
    ...
    if (ret == css_err_none) {
        LOGV("Node(%s) port '%s' already connected once to '%s'",
                     src_node_name.c_str(),
                     src_port_name.c_str(),
                     peerStr.c_str());
    }
    ...
    if (ret != css_err_none) {
        ...
        LOGV("Node(%s) port '%s' already connected once to '%s' adding new peer",
                      dst_node_name.c_str(),
                      dst_port_name.c_str(),
                      peerStr.c_str());
