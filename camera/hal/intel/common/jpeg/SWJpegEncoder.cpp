...
int SWJpegEncoder::encode(const InputBuffer &in, const OutputBuffer &out)
{
    ...
    return mJpegSize;
exit:
    return (mJpegSize = INVALID_FILE_SIZE);
}
...
void SWJpegEncoder::deInit(void)
{
    ...
    for (unsigned int i = 0; i < mSwJpegEncoder.size(); i++) {
        mSwJpegEncoder.at(i).reset();
    }
    mSwJpegEncoder.erase(mSwJpegEncoder.begin(), mSwJpegEncoder.end());
...
int SWJpegEncoder::doJpegEncodingMultiThread(void)
{
    ...
    for (unsigned int i = 0; i <mSwJpegEncoder.size(); i++) {
        ...
        LOG1("@%s, new sw jpeg thread name:%s", __FUNCTION__, threadName.c_str());
        encThread = mSwJpegEncoder.at(i);
        status = encThread->runThread(threadName.c_str());
        if (status != NO_ERROR) {
            LOGE("@%s, line:%d, start jpeg thread fail, thread name:%s", __FUNCTION__, __LINE__, threadName.c_str());
            return status;
        }
    }
...
#define NV12_MCU_SIZE 16
    ...
    // check if there are available sw jpeg encoders
    if (mSwJpegEncoder.size() <= 0)
        return INVALID_FILE_SIZE;

    std::shared_ptr<CodecWorkerThread> encThread = mSwJpegEncoder.at(0);
    if (encThread.get() == nullptr)
        return INVALID_FILE_SIZE;

    encThread->getConfig(&cfg);

    /* Write the JPEG header */
    STDCOPY(mDstBuf, (int8_t *) cfg.outBuf, size);
    ...
    /* Write the SOS */
    STDCOPY((int8_t *)((unsigned long)mDstBuf + size),
            (int8_t *)((unsigned long)cfg.outBuf + HEADER_TOTAL_LEN - HEADER_SOS_LEN),
            HEADER_SOS_LEN);
    size += HEADER_SOS_LEN;
...
{
    LOG1("@%s, line:%d", __FUNCTION__, __LINE__);
    return (pthread_create(&mThreadId, nullptr, threadLoop, this) == 0) ? OK : NO_INIT;
}
 ...
 * \return false
 */
bool SWJpegEncoder::CodecWorkerThread::_threadLoop()
{
