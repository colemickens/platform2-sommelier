                              ...
                              const char* sensorName)
{
    ...
    if (count >= 2 && exposureTimeRange != nullptr) {
        ...
        LOG2("mMinExposureTime:%lld mMaxExposureTime:%lld", mMinExposureTime, mMaxExposureTime);
    }
    ...
    if (count >= 1 && maxFrameDuration != nullptr) {
        ...
        LOG2("mMaxFrameDuration:%lld", mMaxFrameDuration);
    }
        ...
        if (entry.count == 1) {
            ...
            if (timeMicros > 0) {
                if (timeMicros > mMaxExposureTime / 1000) {
                    LOGE("exposure time %lld ms is bigger than the max exposure time %lld ms",
                        timeMicros, mMaxExposureTime / 1000);
                    return BAD_VALUE;
                } else if (timeMicros < mMinExposureTime / 1000) {
                    LOGE("exposure time %lld ms is smaller than the min exposure time %lld ms",
        ...
        if (entry.count == 1) {
            ...
            if (timeMicros > 0) {
                if (timeMicros > mMaxFrameDuration / 1000) {
                    LOGE("frame duration %lld ms is bigger than the max frame duration %lld ms",
