...
int V4L2DeviceBase::poll(int timeout)
{
    ...
    if (mFd == -1) {
        LOG1("Device %s already closed. Do nothing.", mName.c_str());
        return -1;
    }
...
int V4L2DeviceBase::subscribeEvent(int event)
{
    ...
    if (mFd == -1) {
        LOG1("Device %s already closed. cannot subscribe.", mName.c_str());
        return -1;
    }
...
int V4L2DeviceBase::unsubscribeEvent(int event)
{
    ...
    if (mFd == -1) {
        LOG1("Device %s closed. cannot unsubscribe.", mName.c_str());
        return -1;
    }
...
int V4L2DeviceBase::dequeueEvent(struct v4l2_event *event)
{
    ...
    if (mFd == -1) {
        LOG1("Device %s closed. cannot dequeue event.", mName.c_str());
        return -1;
    }
