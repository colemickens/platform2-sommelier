...
bool AssignAddr(const std::string& ifname, DeviceConfig* config) {
  ...
  // maps device names to the base host address (used in kIPv4AddrFmt).
  // TODO(garrick): Generalize and support arbitrary interfaces.
  static const std::map<std::string, int> cur_ifaces = {
      {kAndroidDevice, 1}, {"eth0", 5},    {"wlan0", 9},
      {"wwan0", 13},       {"rmnet0", 17}, {"tun0", 21}};
  const auto it = cur_ifaces.find(ifname);
  if (it == cur_ifaces.end())
    return false;

  const int addr_off = it->second;
  config->set_br_ipv4(base::StringPrintf(kIPv4AddrFmt, addr_off));
  config->set_arc_ipv4(base::StringPrintf(kIPv4AddrFmt, addr_off + 1));
  config->set_mac_addr(
