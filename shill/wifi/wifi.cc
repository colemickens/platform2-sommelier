...
void WiFi::SetHT40EnableForService(const WiFiService* service, bool enable) {
  ...
  if (!supplicant_interface_proxy_->SetHT40Enable(rpcid, enable)) {
    LOG(ERROR) << "SetHT40Enable for " << rpcid.value() << " failed.";
  }
}
