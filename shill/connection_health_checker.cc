...
void ConnectionHealthChecker::OnConnectionComplete(bool success, int sock_fd) {
  ...
  Result result = kResultUnknown;
  if (run_data_test_)
    result = SendData(sock_fd);
  else
    result = ShutDown(sock_fd);
...
bool ConnectionHealthChecker::GetSocketInfo(int sock_fd,
  ...
  if (socket_->GetSockName(sock_fd, addr, &addrlen) != 0) {
    SLOG(Connection, 2) << __func__
  ...
  CHECK(addrlen == sizeof(struct sockaddr_in));
