...
TEST(cgroup_new_with_parent) {
  ...
  mkdir(path, S_IRWXU | S_IRWXG);
  snprintf(
      path, sizeof(path), "%s/cpuacct/%s", cgroup_root, cgroup_parent_name);
  mkdir(path, S_IRWXU | S_IRWXG);
  snprintf(path, sizeof(path), "%s/cpuset/%s", cgroup_root, cgroup_parent_name);
  mkdir(path, S_IRWXU | S_IRWXG);
  snprintf(
      path, sizeof(path), "%s/devices/%s", cgroup_root, cgroup_parent_name);
  mkdir(path, S_IRWXU | S_IRWXG);
  snprintf(
      path, sizeof(path), "%s/freezer/%s", cgroup_root, cgroup_parent_name);
  mkdir(path, S_IRWXU | S_IRWXG);
  snprintf(
      path, sizeof(path), "%s/schedtune/%s", cgroup_root, cgroup_parent_name);
  mkdir(path, S_IRWXU | S_IRWXG);

  snprintf(
      path, sizeof(path), "%s/cpuset/%s/cpus", cgroup_root, cgroup_parent_name);
  create_file_with_content(path, "0-3");
  snprintf(
      path, sizeof(path), "%s/cpuset/%s/mems", cgroup_root, cgroup_parent_name);
  create_file_with_content(path, "0");

  ccg =
      container_cgroup_new(CGNAME, cgroup_root, cgroup_parent_name, 1000, 1000);
...
FIXTURE_SETUP(basic_manipulation) {
  ...
  snprintf(self->cpu_cg,
           sizeof(self->cpu_cg),
           "%s/cpu/%s",
           self->cgroup_root,
           self->cgroup_name);
  snprintf(self->cpuacct_cg,
           sizeof(self->cpuacct_cg),
           "%s/cpuacct/%s",
           self->cgroup_root,
           self->cgroup_name);
  snprintf(self->cpuset_cg,
           sizeof(self->cpuset_cg),
           "%s/cpuset/%s",
           self->cgroup_root,
           self->cgroup_name);
  snprintf(self->devices_cg,
           sizeof(self->devices_cg),
           "%s/devices/%s",
           self->cgroup_root,
           self->cgroup_name);
  snprintf(self->freezer_cg,
           sizeof(self->freezer_cg),
           "%s/freezer/%s",
           self->cgroup_root,
           self->cgroup_name);
  snprintf(self->schedtune_cg,
           sizeof(self->schedtune_cg),
           "%s/schedtune/%s",
           self->cgroup_root,
           self->cgroup_name);
...
TEST_F(basic_manipulation, add_device_invalid_type) {
  EXPECT_NE(0, self->ccg->ops->add_device(self->ccg, 1, 14, 3, 1, 1, 0, 'x'));
}

TEST_F(basic_manipulation, add_device_no_perms) {
  EXPECT_NE(0, self->ccg->ops->add_device(self->ccg, 1, 14, 3, 0, 0, 0, 'c'));
}

TEST_F(basic_manipulation, add_device_rw) {
  char path[256];
  EXPECT_EQ(0, self->ccg->ops->add_device(self->ccg, 1, 14, 3, 1, 1, 0, 'c'));
  snprintf(path, sizeof(path), "%s/devices.allow", self->devices_cg);
...
TEST_F(basic_manipulation, add_device_rwm) {
  char path[256];
  EXPECT_EQ(0, self->ccg->ops->add_device(self->ccg, 1, 14, 3, 1, 1, 1, 'c'));
  snprintf(path, sizeof(path), "%s/devices.allow", self->devices_cg);
...
TEST_F(basic_manipulation, add_device_ro) {
  char path[256];
  EXPECT_EQ(0, self->ccg->ops->add_device(self->ccg, 1, 14, 3, 1, 0, 0, 'c'));
  snprintf(path, sizeof(path), "%s/devices.allow", self->devices_cg);
...
TEST_F(basic_manipulation, add_device_wo) {
  char path[256];
  EXPECT_EQ(0, self->ccg->ops->add_device(self->ccg, 1, 14, 3, 0, 1, 0, 'c'));
  snprintf(path, sizeof(path), "%s/devices.allow", self->devices_cg);
...
TEST_F(basic_manipulation, add_device_major_wide) {
  char path[256];
  EXPECT_EQ(0, self->ccg->ops->add_device(self->ccg, 1, 14, -1, 0, 1, 0, 'c'));
  snprintf(path, sizeof(path), "%s/devices.allow", self->devices_cg);
...
TEST_F(basic_manipulation, add_device_major_minor_wildcard) {
  char path[256];
  EXPECT_EQ(0, self->ccg->ops->add_device(self->ccg, 1, -1, -1, 0, 1, 0, 'c'));
  snprintf(path, sizeof(path), "%s/devices.allow", self->devices_cg);
...
TEST_F(basic_manipulation, add_device_deny_all) {
  char path[256];
  EXPECT_EQ(0, self->ccg->ops->add_device(self->ccg, 0, -1, -1, 1, 1, 1, 'a'));
  snprintf(path, sizeof(path), "%s/devices.deny", self->devices_cg);
...
TEST_F(basic_manipulation, add_device_block) {
  char path[256];
  EXPECT_EQ(0, self->ccg->ops->add_device(self->ccg, 1, 14, 3, 1, 1, 0, 'b'));
  snprintf(path, sizeof(path), "%s/devices.allow", self->devices_cg);
