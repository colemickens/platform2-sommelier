...
void ProfilerWriter::Update(const Profiler& profiler) {
  ...
  // overwrite header
  fwrite(&profiler.max_num_symbols_, sizeof(profiler.max_num_symbols_), 1, fp);
  fwrite(&profiler.num_symbols_, sizeof(profiler.num_symbols_), 1, fp);
  fwrite(&num_written_samples_, sizeof(num_written_samples_), 1, fp);

  if (num_written_symbols_ != profiler.num_symbols_) {
    // overwrite symbols
    fwrite(profiler.symbols_.get(), sizeof(profiler.symbols_),
  ...
  // append samples
  fseek(fp, 0, SEEK_END);
  fwrite(profiler.samples_.get(), sizeof(profiler.samples_),
