...
TEST_F(TestSession, ImportRSAWithTPM) {
  ...
  CK_ATTRIBUTE private_attributes[] = {
    ...
    {CKA_MODULUS, base::string_as_array(&n), n.length() },
    {CKA_PUBLIC_EXPONENT, base::string_as_array(&e), e.length() },
    {CKA_PRIVATE_EXPONENT, base::string_as_array(&d), d.length() },
    {CKA_PRIME_1, base::string_as_array(&p), p.length() },
    {CKA_PRIME_2, base::string_as_array(&q), q.length() },
    {CKA_EXPONENT_1, base::string_as_array(&dmp1), dmp1.length() },
    {CKA_EXPONENT_2, base::string_as_array(&dmq1), dmq1.length() },
    {CKA_COEFFICIENT, base::string_as_array(&iqmp), iqmp.length() },
  };
