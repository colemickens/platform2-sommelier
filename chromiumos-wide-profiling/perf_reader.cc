...
bool PerfReader::ReadFileFromHandle(std::ifstream * in) {
  if (!in->good())
    return false;
  if (ReadHeader(in) == false)
    return false;
  if (ReadAttrs(in) == false)
    return false;
  if (ReadData(in) == false)
    return false;
...
bool PerfReader::ReadData(std::ifstream * in) {
  ...
  while (data_remaining_bytes != 0) {
    perf_event_header peh;
    in->read(reinterpret_cast<char*>(&peh), sizeof(peh));
    if (!in->good())
      return false;

    LOG(INFO) << "Data remaining: " << data_remaining_bytes;
    LOG(INFO) << "Data type: " << peh.type;
    LOG(INFO) << "Data size: " << peh.size;
    size_t remaining_size = peh.size - sizeof(peh);
    LOG(INFO) << "Seek size: " << remaining_size;
    data_remaining_bytes -= peh.size;

    if (peh.size > sizeof(event_t)) {
      LOG(INFO) << "Data size: " << peh.size << " sizeof(event_t): " <<
                sizeof(event_t);
      return false;
    }

    in->seekg(-1L * (long)(sizeof(peh)), std::ios::cur);
    events_.resize(events_.size() + 1);

    in->read(reinterpret_cast<char*>(&(events_.back())), peh.size);
    if (!in->good())
      return false;
    ...
    if (peh.type == PERF_RECORD_COMM) {
      LOG(INFO) << "len: " << peh.size;
      LOG(INFO) << "sizeof header: " << sizeof(peh);
...
bool PerfReader::ReadHeader(std::ifstream * in) {
  ...
  if (!in->good())
    return false;
  if (header_.magic != PERF_MAGIC) {
    LOG(ERROR) << "Read wrong magic. Expected: " << PERF_MAGIC <<
               " Got: " << header_.magic;
    return false;
  }
...
bool PerfReader::WriteEventFull(std::ofstream * out,
                                const event_t * eventp) const {
  out->write(reinterpret_cast<const char*>(eventp), eventp->header.size);
  return out->good();
}
...
bool PerfReader::WriteAttrs(std::ofstream * out) const {
  ...
  for ( size_t i = 0 ; i < attrs_.size() ; i++ ) {
    ...
    out->write(reinterpret_cast<const char*>(&ids),
               sizeof(ids));
    if (!out->good())
      return false;
    ids_size += ids.size;
